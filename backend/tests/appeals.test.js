...
describe('Appeals API Tests', () => {
  beforeAll(async () => {
    console.log('\n[Appeal Tests] Starting MongoDB connection...');
    ...
    if (mongoose.connection.readyState !== 1) {
      throw new Error(
        `MongoDB connection was not established. Ready state: ${mongoose.connection.readyState}`
      );
    }

    console.log(
      `[Appeal Tests] MongoDB ready. Ready state: ${mongoose.connection.readyState}`
    );
    ...
    console.log(`[Appeal Tests] Test report created: ${reportId}`);
  });
