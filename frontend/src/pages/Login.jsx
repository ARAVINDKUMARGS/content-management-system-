...
const Login = () => {
  ...
  const [selectedRole, setSelectedRole] = useState(
    location.state?.registeredRole || 'admin'
  );
  const [email, setEmail] = useState(
    location.state?.registeredEmail || 'admin@lumen.com'
  );
  const [password, setPassword] = useState(
    location.state?.registeredEmail ? '' : 'password123'
  );
  ...
  const handleQuickDemoLogin = async (demoEmail, demoRole) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(
          res.message || 'Failed to sign in with demo credentials.'
        );
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        'Failed to connect to authentication service. Please check your connection.'
      );
    } finally {
      setLoading(false);
    }
